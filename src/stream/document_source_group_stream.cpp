...
/**
 * Generates a new file name on each call using a static, atomic and
 * monotonically increasing number.
 *
 * Each user of the Sorter must implement this function to ensure that all
 * temporary files that the Sorter instances produce are uniquely identified
 * using a unique file name extension with separate atomic variable. This is
 * necessary because the sorter.cpp code is separately included in multiple
 * places, rather than compiled in one place and linked, and so cannot provide a
 * globally unique ID.
 */
std::string nextFileName() {
  static AtomicWord<unsigned> documentSourceGroupFileCounter;
  return "extsort-doc-group." +
         std::to_string(documentSourceGroupFileCounter.fetchAndAdd(1));
}
...
Document GroupFromFirstDocumentTransformationStream::applyTransformation(
    const Document &input) {
  MutableDocument output(_accumulatorExprs.size());

  for (auto &&expr : _accumulatorExprs) {
    auto value = expr.second->evaluate(
        input, &expr.second->getExpressionContext()->variables);
    output.addField(expr.first,
                    value.missing() ? Value(BSONNULL) : std::move(value));
  }

  return output.freeze();
}

void GroupFromFirstDocumentTransformationStream::optimize() {
  for (auto &&expr : _accumulatorExprs) {
    expr.second = expr.second->optimize();
  }
}

Document GroupFromFirstDocumentTransformationStream::serializeTransformation(
    boost::optional<ExplainOptions::Verbosity> explain) const {

  MutableDocument newRoot(_accumulatorExprs.size());
  for (auto &&expr : _accumulatorExprs) {
    newRoot.addField(expr.first,
                     expr.second->serialize(static_cast<bool>(explain)));
  }

  return {{"newRoot", newRoot.freezeToValue()}};
}

DepsTracker::State GroupFromFirstDocumentTransformationStream::addDependencies(
    DepsTracker *deps) const {
  for (auto &&expr : _accumulatorExprs) {
    expr.second->addDependencies(deps);
  }

  // This stage will replace the entire document with a new document, so any
  // existing fields will be replaced and cannot be required as dependencies. We
  // use EXHAUSTIVE_ALL here instead of EXHAUSTIVE_FIELDS, as in
  // ReplaceRootTransformation, because the stages that follow a $group stage
  // should not depend on document metadata.
  return DepsTracker::State::EXHAUSTIVE_ALL;
}

DocumentSource::GetModPathsReturn
GroupFromFirstDocumentTransformationStream::getModifiedPaths() const {
...
std::unique_ptr<GroupFromFirstDocumentTransformationStream>
GroupFromFirstDocumentTransformationStream::create(
    const intrusive_ptr<ExpressionContext> &expCtx, const std::string &groupId,
    vector<pair<std::string, intrusive_ptr<Expression>>> accumulatorExprs) {
  return std::make_unique<GroupFromFirstDocumentTransformationStream>(
      groupId, std::move(accumulatorExprs));
}

constexpr StringData DocumentSourceGroupStream::kStageName;

REGISTER_DOCUMENT_SOURCE(groups, LiteParsedDocumentSourceDefault::parse,
                         DocumentSourceGroupStream::createFromBson,
                         AllowedWithApiStrict::kAlways);

const char *DocumentSourceGroupStream::getSourceName() const {
  return kStageName.rawData();
}

bool DocumentSourceGroupStream::shouldSpillWithAttemptToSaveMemory() {
...
void DocumentSourceGroupStream::freeMemory() {
  invariant(!_states.front()._groups.empty());
  for (auto &&group : _states.front()._groups) {
...
DocumentSource::GetNextResult DocumentSourceGroupStream::getNextSpilled() {
  ...
  if (!_states.front()._sorterIterator) {
    return GetNextResult::makeEOF();
  }

  _states.front()._currentId = _states.front()._firstPartOfNextGroup.first;
  const size_t numAccumulators = _accumulatedFields.size();

  // Call startNewGroup on every accumulator.
  Value expandedId = expandId(_states.front()._currentId);
  ...
  for (size_t i = 0; i < numAccumulators; ++i) {
    ...
    _states.front()._currentAccumulators[i]->startNewGroup(initializerValue);
  }

  while (pExpCtx->getValueComparator().evaluate(
      _states.front()._currentId ==
      _states.front()._firstPartOfNextGroup.first)) {
    ...
    case 1: // Single accumulators serialize as a single Value.
      _states.front()._currentAccumulators[0]->process(
          _states.front()._firstPartOfNextGroup.second, true);
      [[fallthrough]];
    ...
    default: { // Multiple accumulators serialize as an array of Values.
      const vector<Value> &accumulatorStates =
          _states.front()._firstPartOfNextGroup.second.getArray();
      for (size_t i = 0; i < numAccumulators; i++) {
        _states.front()._currentAccumulators[i]->process(accumulatorStates[i],
                                                         true);
      }
    }
    }

    if (!_states.front()._sorterIterator->more()) {
      doDispose(); // reseting states, not really disposing
      break;
    }

    _states.front()._firstPartOfNextGroup =
        _states.front()._sorterIterator->next();
  }

  return makeDocument(_states.front()._currentId,
                      _states.front()._currentAccumulators,
                      pExpCtx->needsMerge);
}
...
intrusive_ptr<DocumentSource> DocumentSourceGroupStream::optimize() {
  // Optimizing a 'DocumentSourceGroup' might modify its expressions to become
  // incompatible with SBE. We temporarily highjack the context's
  // 'sbeCompatible' flag to communicate the situation back to the
  // 'DocumentSourceGroup'. Notice, that while a particular
  // 'DocumentSourceGroup' might become incompatible with SBE, other groups in
  // the pipeline and the collection access could be still eligible for lowering
  // to SBE, thus we must reset the context's 'sbeCompatible' flag back to its
  // original value at the end of the 'optimize()' call.
  //
  // TODO SERVER-XXXXX: replace this hack with a proper per-stage tracking of
  // SBE compatibility.
  ...
  // then we know there will be only one group. We should take advantage of that
  // to avoid going through the hash table.
  ...
  for (auto &&accumulatedField : _accumulatedFields) {
    accumulatedField.expr.initializer =
        accumulatedField.expr.initializer->optimize();
...
Value DocumentSourceGroupStream::serialize(
    boost::optional<ExplainOptions::Verbosity> explain) const {
  ...
  if (_idFieldNames.empty()) {
    ...
    for (size_t i = 0; i < _idExpressions.size(); i++) {
      md[_idFieldNames[i]] =
          _idExpressions[i]->serialize(static_cast<bool>(explain));
    }
  ...
  for (auto &&accumulatedField : _accumulatedFields) {
    ...
    insides[accumulatedField.fieldName] = Value(accum->serialize(
        accumulatedField.expr.initializer, accumulatedField.expr.argument,
        static_cast<bool>(explain)));
  }

  if (_doingMerge) {
    // This makes the output unparsable (with error) on pre 2.6 shards, but it
    // will never be sent to old shards when this flag is true since they can't
    // do a merge anyway.
...
DocumentSource::GetModPathsReturn
DocumentSourceGroupStream::getModifiedPaths() const {
  // We preserve none of the fields, but any fields referenced as part of the
  // group key are logically just renamed.
...
StringMap<boost::intrusive_ptr<Expression>>
DocumentSourceGroupStream::getIdFields() const {
  if (_idFieldNames.empty()) {
    invariant(_idExpressions.size() == 1);
...
const std::vector<AccumulationStatement> &
DocumentSourceGroupStream::getAccumulatedFields() const {
  return _accumulatedFields;
}

intrusive_ptr<DocumentSourceGroupStream> DocumentSourceGroupStream::create(
    const intrusive_ptr<ExpressionContext> &expCtx,
    const boost::intrusive_ptr<Expression> &groupByExpression,
    std::vector<AccumulationStatement> accumulationStatements,
    boost::optional<size_t> maxMemoryUsageBytes) {
  intrusive_ptr<DocumentSourceGroupStream> groupStage(
      new DocumentSourceGroupStream(expCtx, maxMemoryUsageBytes));
...
DocumentSourceGroupStream::DocumentSourceGroupStream(
    const intrusive_ptr<ExpressionContext> &expCtx,
    boost::optional<size_t> maxMemoryUsageBytes)
    : DocumentSource(kStageName, expCtx), _doingMerge(false),
      _memoryTracker{
          expCtx->allowDiskUse && !expCtx->inMongos,
          maxMemoryUsageBytes
              ? *maxMemoryUsageBytes
              : static_cast<size_t>(
                    internalDocumentSourceGroupMaxMemoryBytes.load())},
      // We spill to disk in debug mode, regardless of allowDiskUse, to stress
      // the system.
...
void DocumentSourceGroupStream::addAccumulator(
    AccumulationStatement accumulationStatement) {
...
parseIdExpression(const intrusive_ptr<ExpressionContext> &expCtx,
                  BSONElement groupField, const VariablesParseState &vps) {
...
void DocumentSourceGroupStream::setIdExpression(
    const boost::intrusive_ptr<Expression> idExpression) {
  if (auto object = dynamic_cast<ExpressionObject *>(idExpression.get())) {
    auto &childExpressions = object->getChildExpressions();
    invariant(
        !childExpressions.empty()); // We expect to have converted an empty
                                    // object into a constant expression.

    // grouping on an "artificial" object. Rather than create the object for
    // each input in initialize(), instead group on the output of the raw
    // expressions. The artificial object will be created at the end in
    // makeDocument() while outputting results.
...
DocumentSourceGroupStream::getIdExpression() const {
  // _idFieldNames is empty and _idExpressions has one element when the _id
  // expression is not an object expression.
  ...
  tassert(6586300,
          ...
          _idFieldNames.size() > 0 &&
              _idFieldNames.size() == _idExpressions.size());

  // Each expression in '_idExpressions' may have been optimized and so, compose
  // the object _id expression out of the optimized expressions.
...
intrusive_ptr<DocumentSource> DocumentSourceGroupStream::createFromBson(
  ...
  intrusive_ptr<DocumentSourceGroupStream> groupStage(
      new DocumentSourceGroupStream(expCtx));
  ...
  while (groupIterator.more()) {
    ...
    if (pFieldName == "_id") {
      ...
      groupStage->addAccumulator(
          AccumulationStatement::parseAccumulationStatement(expCtx.get(),
                                                            groupField, vps));
      groupStage->_memoryTracker.set(pFieldName, 0);
    }
  }
  groupStage->_sbeCompatible =
      expCtx->sbeGroupCompatible && expCtx->sbeCompatible;
...
using GroupsMap = DocumentSourceGroupStream::GroupsMap;

class SorterComparator {
public:
  typedef pair<Value, Value> Data;

  SorterComparator(ValueComparator valueComparator)
...
public:
  SpillSTLComparator(ValueComparator valueComparator)
      : _valueComparator(valueComparator) {}

  bool operator()(const GroupsMap::value_type *lhs,
                  const GroupsMap::value_type *rhs) const {
...
DocumentSource::GetNextResult DocumentSourceGroupStream::initialize() {
...
// initialize() and prevent stack overflows.
MONGO_COMPILER_NOINLINE DocumentSource::GetNextResult
DocumentSourceGroupStream::initializeSelf(GetNextResult input) {
  const size_t numAccumulators = _accumulatedFields.size();
  ...
  case DocumentSource::GetNextResult::ReturnStatus::kUnblock: {
    LOGV2(99999, "unblocked");
    // Do any final steps necessary to prepare to output results.
    if (!state._sortedFiles.empty()) {
      state._spilled = true;
      if (!state._groups.empty()) {
      ...
      state._sorterIterator.reset(Sorter<Value, Value>::Iterator::merge(
          state._sortedFiles, SortOptions(),
          SorterComparator(pExpCtx->getValueComparator())));

      // prepare current to accumulate data
      state._currentAccumulators.reserve(numAccumulators);
      for (auto &&accumulatedField : _accumulatedFields) {
        state._currentAccumulators.push_back(
            accumulatedField.makeAccumulator());
      }

      verify(state._sorterIterator
                 ->more()); // we put data in, we should get something out.
      state._firstPartOfNextGroup = state._sorterIterator->next();
    } else {
      // start the group iterator
      state._groupsIterator = state._groups.begin();
    }
...
shared_ptr<Sorter<Value, Value>::Iterator> DocumentSourceGroupStream::spill() {
  ...
  ptrs.reserve(state._groups.size());
  for (GroupsMap::const_iterator it = state._groups.begin(),
                                 end = state._groups.end();
       it != end; ++it) {
    ptrs.push_back(&*it);
  }

  stable_sort(ptrs.begin(), ptrs.end(),
              SpillSTLComparator(pExpCtx->getValueComparator()));

  SortedFileWriter<Value, Value> writer(SortOptions().TempDir(pExpCtx->tempDir),
                                        _file);
  switch (
      _accumulatedFields.size()) { // same as ptrs[i]->second.size() for all i.
  ...
  case 1: // just one value, use optimized serialization as single Value
    for (size_t i = 0; i < ptrs.size(); i++) {
      writer.addAlreadySorted(
          ptrs[i]->first, ptrs[i]->second[0]->getValue(/*toBeMerged=*/true));
    }
    break;
  ...
  auto &metricsCollector =
      ResourceConsumption::MetricsCollector::get(pExpCtx->opCtx);
  metricsCollector.incrementKeysSorted(ptrs.size());
  metricsCollector.incrementSorterSpills(1);

  state._groups.clear();
  // Zero out the current per-accumulation statement memory consumption, as the
  // memory has been freed by spilling.
...
Value DocumentSourceGroupStream::computeId(const Document &root) {
...
Value DocumentSourceGroupStream::expandId(const Value &val) {
...
Document DocumentSourceGroupStream::makeDocument(const Value &id,
...
DocumentSourceGroupStream::distributedPlanLogic() {
...
bool DocumentSourceGroupStream::pathIncludedInGroupKeys(
    const std::string &dottedPath) const {
  return std::any_of(_idExpressions.begin(), _idExpressions.end(),
                     [&dottedPath](const auto &exp) {
                       if (auto fieldExp =
                               dynamic_cast<ExpressionFieldPath *>(exp.get())) {
...
bool DocumentSourceGroupStream::canRunInParallelBeforeWriteStage(
    const std::set<std::string> &nameOfShardKeyFieldsUponEntryToStage) const {
  if (_doingMerge) {
    return true; // This is fine.
  }

  // Certain $group stages are allowed to execute on each exchange consumer. In
  // order to guarantee each consumer will only group together data from its own
  // shard, the $group must group on a superset of the shard key.
  for (auto &&currentPathOfShardKey : nameOfShardKeyFieldsUponEntryToStage) {
    if (!pathIncludedInGroupKeys(currentPathOfShardKey)) {
      // This requires an exact path match, but as a future optimization certain
      // path prefixes should be okay. For example, if the shard key path is
      // "a.b", and we're grouping by "a", then each group of "a" is strictly
      // more specific than "a.b", so we can deduce that grouping by "a" will
      // not need to group together documents across different values of the
      // shard key field "a.b", and thus as long as any other shard key fields
      // are similarly preserved will not need to consume a merged stream to
      // perform the group.
      return false;
    }
  }
...
std::unique_ptr<GroupFromFirstDocumentTransformationStream>
DocumentSourceGroupStream::rewriteGroupAsTransformOnFirstDocument() const {
  if (_idExpressions.size() != 1) {
    // This transformation is only intended for $group stages that group on a
    // single field.
    return nullptr;
  }

  auto fieldPathExpr =
      dynamic_cast<ExpressionFieldPath *>(_idExpressions.front().get());
  ...
  if (fieldPath.getPathLength() == 1) {
    // The path is $$CURRENT or $$ROOT. This isn't really a sensible value to
    // group by (since each document has a unique _id, it will just return the
    // entire collection). We only apply the rewrite when grouping by a single
    // field, so we cannot apply it in this case, where we are grouping by the
    // entire document.
    tassert(
        5943200,
        "Optimization attempted on group by always-dissimilar system variable",
        fieldPath.getFieldName(0) == "CURRENT" ||
            fieldPath.getFieldName(0) == "ROOT");
    return nullptr;
  }
  ...
  // The _id field can be specified either as a fieldpath (ex. _id: "$a") or as
  // a singleton object (ex. _id: {v: "$a"}).
  if (_idFieldNames.empty()) {
    ...
    idField = ExpressionObject::create(
        pExpCtx.get(), {{_idFieldNames.front(), _idExpressions.front()}});
  }
  fields.push_back(std::make_pair("_id", idField));

  for (auto &&accumulator : _accumulatedFields) {
    fields.push_back(
        std::make_pair(accumulator.fieldName, accumulator.expr.argument));

    // Since we don't attempt this transformation for non-$first accumulators,
    // the initializer should always be trivial.
  }

  return GroupFromFirstDocumentTransformationStream::create(pExpCtx, groupId,
                                                            std::move(fields));
}
...
// Explicit instantiation unneeded since we aren't exposing Sorter outside of
// this file.
