...
#include "document_source_random_delay.h"
...
REGISTER_DOCUMENT_SOURCE(randomDelay, LiteParsedDocumentSourceDefault::parse,
...
intrusive_ptr<DocumentSource> DocumentSourceRandomDelay::createFromBson(
  ...
  uassert(99999, str::stream() << "the max delay field must be an integer",
          elem.type() == NumberInt);
