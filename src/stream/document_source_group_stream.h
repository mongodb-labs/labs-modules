...
/**
 * GroupFromFirstTransformation consists of a list of (field name, expression
 * pairs). It returns a document synthesized by assigning each field name in the
 * output document to the result of evaluating the corresponding expression. If
 * the expression evaluates to missing, we assign a value of BSONNULL. This is
 * necessary to match the semantics of $first for missing fields.
 */
class GroupFromFirstDocumentTransformationStream final
    : public TransformerInterface {
public:
  GroupFromFirstDocumentTransformationStream(
  ...
  /**
   * The path of the field that we are grouping on: i.e., the field in the input
   * document that we will use to create the _id field of the ouptut document.
   */
  ...
  static std::unique_ptr<GroupFromFirstDocumentTransformationStream>
...
public:
  ...
  /**
   * Convenience method for creating a new $group stage. If maxMemoryUsageBytes
   * is boost::none, then it will actually use the value of
   * internalDocumentSourceGroupMaxMemoryBytes.
   */
  static boost::intrusive_ptr<DocumentSourceGroupStream>
  create(const boost::intrusive_ptr<ExpressionContext> &expCtx,
  ...
  /**
   * Add an accumulator, which will become a field in each Document that results
   * from grouping.
   */
  ...
  /**
   * Returns true if this $group stage represents a 'global' $group which is
   * merging together results from earlier partial groups.
   */
  bool doingMerge() const { return _doingMerge; }
  ...
  /**
   * Returns true if this $group stage used disk during execution and false
   * otherwise.
   */
  bool usedDisk() final { return _stats.spills > 0; }

  const SpecificStats *getSpecificStats() const final { return &_stats; }
  ...
  /**
   * When possible, creates a document transformer that transforms the first
   * document in a group into one of the output documents of the $group stage.
   * This is possible when we are grouping on a single field and all
   * accumulators are $first (or there are no accumluators).
   *
   * It is sometimes possible to use a DISTINCT_SCAN to scan the first document
   * of each group, in which case this transformation can replace the actual
   * $group stage in the pipeline (SERVER-9507).
   */
...
private:
   ...
   * before being called, and also expect initialize() to have been called
   * already.
   */
  ...
  /**
   * Initializes this $group after any children are potentially initialized see
   * initialize() for more details.
   */
  GetNextResult initializeSelf(GetNextResult input);

  /**
   * Spill groups map to disk and returns an iterator to the file. Note: Since a
   * sorted $group does not exhaust the previous stage before returning, and
   * thus does not maintain as large a store of documents at any one time, only
   * an unsorted group can spill to disk.
   */
  std::shared_ptr<Sorter<Value, Value>::Iterator> spill();

  /**
   * If we ran out of memory, finish all the pending operations so that some
   * memory can be freed.
   */
  void freeMemory();
  ...
  /**
   * Converts the internal representation of the group key to the _id shape
   * specified by the user.
   */
  Value expandId(const Value &val);

  /**
   * Returns true if 'dottedPath' is one of the group keys present in
   * '_idExpressions'.
   */
  ...
  // If the expression for the '_id' field represents a non-empty object, we
  // track its fields' names in '_idFieldNames'.
  std::vector<std::string> _idFieldNames;
  // Expressions for the individual fields when '_id' produces a document in the
  // order of
